      ...
      rationale: {
        ...
        properties: {
          photoPlan: {
            ...
            description:
              "이 시뮬레이션에 이미지 자료가 필요한지에 대한 판단과 그 근거. 필요 없다고 판단한 경우에도 반드시 이유를 기록.",
            properties: {
              needed: {
                type: "boolean",
                description:
                  "이미지가 있으면 지원자가 상황을 더 빠르고 정확하게 이해할 수 있는지 여부. 직무 종류로 판단하지 말 것 — 디자인·MD처럼 시각물을 다루는 직무만 해당하는 것이 아니다. 마케팅이라면 경쟁사 광고 크리에이티브·랜딩페이지 화면·매장 POP, CRM이라면 푸시 알림이나 앱 화면, 기획이라면 서비스 플로우 화면이나 대시보드처럼, 글로 길게 설명해야 할 상황을 이미지 한 장이 대신할 수 있으면 true. 반대로 수치·정책·텍스트 자료만으로 충분한 미션이면 false.",
              },
              reason: {
                type: "string",
                ...
                items: {
                  ...
                  properties: {
                    ...
                    description: {
                      type: "string",
                      description:
                        "이 이미지를 새로 만들 수 있을 만큼 구체적인 묘사. 보는 사람이 그대로 제작할 수 있어야 한다. 다음을 모두 담을 것: (1) 화면에 담기는 주요 대상과 그 배치, (2) 배경과 주변 요소, (3) 이미지 안에 들어갈 텍스트·숫자·라벨이 있다면 그 실제 내용, (4) 정보량과 복잡도(단순한지, 요소가 빽빽한지), (5) 색감과 분위기, (6) 각도와 화면 비율. 실사 사진이면 촬영 구도를, 화면 캡처나 광고 소재면 UI 구성과 카피 문구를 그대로 적는다. '매장 사진', '그래프 이미지' 같은 한 줄 요약은 금지.",
                    },
                    purpose: {
                      type: "string",
