...
function GlobalNavigationOverlay() {
  ...
  return (
    <LoadingOverlay
      message={showNavigationLoading ? "페이지 이동 중..." : "불러오는 중..."}
    />
  );
}
...
function SiteLayout({ children }: { children: React.ReactNode }) {
  ...
  const isSimulationRun =
    pathname.startsWith("/simulation/") && !pathname.endsWith("/feedback");
  const isBareFullscreen =
    isHome || pathname.startsWith("/lp") || isSimulationRun;
  const usesOwnHeader =
    isBareFullscreen ||
    ...
    head: () => ({
      meta: [
        ...
        { name: "application-name", content: "Beginner" },
        {
          name: "description",
          content:
            "Beginner는 실제 직무를 온라인으로 경험할 수 있는 직무 시뮬레이션 서비스입니다.",
        },
        ...
        {
          property: "og:image",
          content: "https://beginner.today/og-image.png",
        },
        {
          name: "twitter:image",
          content: "https://beginner.today/og-image.png",
