...
as $$
  with live_applicants as (
    select
      s.id,
      js.company_id,
      coalesce(nullif(r.basics->>'name', ''), split_part(seeker.email, '@', 1), '이름 미입력') as name,
      coalesce(nullif(r.target_role, ''), nullif(js.job_family, ''), js.title, '지원 직무 미입력') as role,
      ...
      case
        ...
        when s.duration_sec < 60 then s.duration_sec::text || '초'
        else (s.duration_sec / 60)::int::text || '분 ' || (s.duration_sec % 60)::text || '초'
      end as duration,
      jsonb_build_array(
        jsonb_build_object(
          'step', 1,
  ...
  seeded_applicants as (
    select
      ...
      a.created_at,
      a.updated_at,
      '5,000만원 협의 가능'::text as desired_salary,
      a.location as preferred_region,
      '정규직, 하이브리드'::text as employment_type,
      '샘플 지원서'::text as resume_title,
      'seed'::text as resume_source_type
    from public.applicants a
    join public.companies c on c.id = a.company_id
    where c.code = company_code
  )
  select * from live_applicants
  union all
  select * from seeded_applicants
